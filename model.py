...
class WSConvTranspose2d(nn.Module):
    """
    Weight scaled ConvTranspose2d (Equalized Learning Rate)
    Note that input is multiplied rather than changing weights
    this will have the same result.
    """

    def __init__(
        self, in_channels, out_channels, kernel_size=4, stride=1, padding=0, gain=1/8
    ):
...
class toRGB(nn.Module):
    def __init__(self, in_channels, img_channels, kernel_size=1, stride=1, padding=0, gain=1):
        super(toRGB, self).__init__()
        self.conv1 = WSConv2d(in_channels, img_channels, kernel_size, stride, padding, gain)
...
class Generator(nn.Module):
    def __init__(self, z_dim, in_channels, img_channels=3):
        ...
        self.initial = nn.Sequential(
            WSConvTranspose2d(z_dim, in_channels, kernel_size=4, stride=1, padding=0, gain=1/8),
            nn.LeakyReLU(0.2),
            PixelNorm(),
        ...
        self.initial_rgb = toRGB(in_channels, img_channels)
        ...
        for i in range(len(factors) - 1):  # -1 to prevent index error because of factors[i+1]
            ...
            self.rgb_layers.append(toRGB(conv_out_c, img_channels))
...
class Discriminator(nn.Module):
    def __init__(self, z_dim, in_channels, img_channels=3):
        ...
        self.final_block = nn.Sequential(
            ...
            nn.LeakyReLU(0.2),
            WSConv2d(
                in_channels, 1, kernel_size=1, padding=0, stride=1, gain = 1,
