...
do
    echo $layer
    save_dir="Results/Compression/Base"
    echo $save_dir
    sbatch --output=Results/%x_%j.out --error=Results/%x_%j.err --time=1-0:0:0 --job-name="compress" --export=layer=$layer,save_dir=$save_dir TestVsRanks.sh 
done